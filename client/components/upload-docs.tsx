...
export default function UploadDocuments() {
  ...
  if (!address) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <p className="text-gray-600">Connect your wallet to upload documents</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-2xl font-bold mb-4 text-black">Upload KYC Documents</h2>
      <p className="text-sm text-gray-600 mb-6">Upload your documents to IPFS. These will be verified by the KCC issuer before you can apply for loans.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="file" accept=".jpg,.jpeg,.png,.pdf" onChange={(e) => handleFileChange(e, 'aadhaar')} className="w-full p-2 border rounded text-black" required />
        ...
        <div>
          ...
          <input type="file" accept=".jpg,.jpeg,.png,.pdf" onChange={(e) => handleFileChange(e, 'landDoc')} className="w-full p-2 border rounded text-black" required />
        ...
        <div>
          ...
          <input type="file" accept=".jpg,.jpeg,.png,.pdf" onChange={(e) => handleFileChange(e, 'incomeProof')} className="w-full p-2 border rounded text-black" required />
          {files.incomeProof && <p className="text-xs text-green-600 mt-1">✓ {files.incomeProof.name}</p>}
        </div>

        <button type="submit" disabled={uploading} className="w-full bg-green-600 text-white p-3 rounded hover:bg-green-700 disabled:bg-gray-400 font-semibold">
          {uploading ? 'Uploading...' : 'Upload Documents to IPFS'}
        </button>

        {status && (
          <div className={`p-3 rounded text-sm ${status.includes('successfully') ? 'bg-green-50 text-green-800' : status.includes('Error') ? 'bg-red-50 text-red-800' : 'bg-blue-50 text-blue-800'}`}>
            {status}
          </div>
        )}
      </form>
